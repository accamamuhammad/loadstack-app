...
export default function InstallCommand({ darkMode, command, selectedCount, totalCount }) {
  ...
  return (
    <div className={`p-6 rounded-lg border ${
      ...
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-xl font-bold">Installation Command</h3>
      ...
      {selectedCount === 0 ? (
        <p className={`text-center py-8 ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>
          ⚠️ No extensions selected. Please select at least one extension to generate the install command.
        </p>
      ) : (
        <>
          <p className={`mb-4 ${darkMode ? 'text-gray-400' : 'text-gray-700'}`}>
            Copy and paste this command into your terminal to install {selectedCount} extension{selectedCount !== 1 ? 's' : ''}:
          </p>
