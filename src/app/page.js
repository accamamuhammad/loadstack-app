...
export default function Home() {
  ...
  const handleSelectLevel = (level) => {
    ...
    if (selectedStack && extensionData[selectedStack]) {
      const stackData = extensionData[selectedStack];
      const levelData = stackData[level];

      if (levelData) {
        setSelectedExtensions(
          Array.from({ length: levelData.length }, (_, i) => i)
        );
      }
    }
  };
  ...
  const selectAllExtensions = () => {
    if (selectedStack && selectedLevel && extensionData[selectedStack]) {
      const levelData = extensionData[selectedStack][selectedLevel];
      if (levelData) {
        setSelectedExtensions(
          Array.from({ length: levelData.length }, (_, i) => i)
        );
  ...
  const getInstallCommand = () => {
    if (!selectedStack || !selectedLevel || selectedExtensions.length === 0)
      return "";
    if (!extensionData[selectedStack]) return "";

    const levelData =
      extensionData[selectedStack][selectedLevel];

    if (!levelData) return "";

    const selectedExts = selectedExtensions.map((idx) => levelData[idx]);
  ...
  return (
    ...
    >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {selectedStack && selectedLevel && extensionData[selectedStack] && (
          <div>
            <button
              onClick={() => {
                setSelectedLevel(null);
